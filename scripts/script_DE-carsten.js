...
function selectShape(event) {
    ...
    updateInfoWindow(`Spieler ${currentPlayer}:<br>Wo möchtest Du diese Form auf dem Spielfeld platzieren?`, clonedShape);
...
function placeShape(event) {
    if (selectedShape) {
        ...
        // Show confirm button
        updateInfoWindow(`<button id="confirm-placement">Spieler ${currentPlayer}:<br>Bist Du fertig?</button>`); 
...
function confirmPlacement() {
    ...
    if (checkWinCondition()) {
        updateInfoWindow(`Spieler ${currentPlayer} gewinnt!<br><button id="play-again">Nochmal spielen</button>`);
        ...
        if (turnCounter === maxTurns) {
            updateInfoWindow(`Es ist ein Unentschieden!<br><button id="play-again">Nochmal spielen</button>`);
            document.getElementById('play-again').addEventListener('click', resetGame);
            gameOver = true; // Set gameOver to true
        } else {
            // Switch player
            currentPlayer = currentPlayer === 1 ? 2 : 1;
            updateInfoWindow(`Spieler ${currentPlayer}:<br>Bitte wähle deine nächste Form aus.`);
        }
    }
...
function resetGame() {
    ...
    updateInfoWindow(`Willkommen zu Carsten's Cube!<br>Spieler 1: Bitte wähle deine erste Form aus.`);
}
